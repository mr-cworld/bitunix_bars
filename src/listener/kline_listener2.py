...
class KlineListener:
    ...
    def process_notification(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Normalize timeframe before creating Indicator instance
            ...
            try:
                # Create Indicator instance with normalized timeframe
                ind = Indicator(historical_df, self.symbol, timeframe)
                ...
                ind.cci()
                ind.roc(period=12)
                ind.momentum(period=14)
                ind.macd()
                ind.ichimoku()
                ind.keltner_channels()
                ind.donchian_channels()
                ind.williams_r(period=14)
                
                # Get the latest values
                latest = ind.get_latest_indicators()
                
                # Create results dictionary with the latest values
                results = {
                    ...
                    'cci': float(latest['cci_20']),  # Note the period difference
                    'roc_12': float(latest['roc_12']),
                    'momentum_14': float(latest['momentum_14']),
                    'williams_r_14': float(latest['williams_r_14']),
                    'macd_line': float(latest['macd_line']),
                    'macd_signal': float(latest['macd_signal']),
                    'macd_histogram': float(latest['macd_histogram']),
                    'ichimoku_tenkan': float(latest['ichimoku_tenkan']),
                    'ichimoku_kijun': float(latest['ichimoku_kijun']),
                    'ichimoku_senkou_a': float(latest.get('ichimoku_senkou_a', 0)),
                    'ichimoku_senkou_b': float(latest.get('ichimoku_senkou_b', 0)),
                    'ichimoku_chikou': float(latest.get('ichimoku_chikou', 0)),
                    'keltner_upper': float(latest['keltner_upper']),
                    'keltner_middle': float(latest['keltner_middle']),
                    'keltner_lower': float(latest['keltner_lower']),
                    'donchian_upper': float(latest['donchian_upper_20']),  # Note the period in column name
                    'donchian_middle': float(latest['donchian_middle_20']),
                    'donchian_lower': float(latest['donchian_lower_20'])
                }
